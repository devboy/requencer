...
### Strategy 1b: Generated Validation Build (System-Level Cross-Board Check)

The shared connector module (Strategy 1) ensures consistent signal *naming*, but each board still builds independently — atopile never sees the full circuit. A **validation build** closes this gap by adding a third build entry that wires both boards together through the connector interface:

```ato
# hardware/boards/elec/src/system.ato — validation only, never fabricated
...
module System:
    ...
    control.connector.spi0_mosi ~ main.connector.spi0_mosi
    control.connector.spi0_miso ~ main.connector.spi0_miso
    control.connector.spi0_sck ~ main.connector.spi0_sck
    # ... all signals
```

This gives atopile a single compilation unit containing the entire circuit: MCU → DACs → op-amps → connector → jacks. End-to-end constraint checking works across both boards.

**The sync problem:** Maintaining `system.ato` by hand defeats the purpose — a forgotten signal means the validation is incomplete. Instead, **generate it from the connector interface definition:**

```python
# hardware/boards/scripts/gen_validation.py
"""
Generate system.ato validation module from BoardConnectorInterface.

Parses board-connector.ato to extract all signal names, then emits a
system.ato that wires control.connector.<signal> ~ main.connector.<signal>
for every signal. Run as a pre-build step.
"""
import re
from pathlib import Path

CONNECTOR_SRC = Path(__file__).parent.parent / "elec/src/board-connector.ato"
SYSTEM_DST = Path(__file__).parent.parent / "elec/src/system.ato"

HEADER = '''\
# AUTO-GENERATED by scripts/gen_validation.py — do not edit manually.
# Regenerate with: python scripts/gen_validation.py
#
# Virtual system module for cross-board validation.
# Wires both boards together through the connector interface so atopile
# can run end-to-end constraint checks. Never fabricated.
...
def extract_signals(src: Path) -> list[str]:
    """Extract signal names from BoardConnectorInterface module."""
    signals = []
    in_module = False
    for line in src.read_text().splitlines():
        if "module BoardConnectorInterface:" in line:
            in_module = True
            continue
        if in_module:
            # Stop at next top-level definition or end of file
            if line and not line[0].isspace() and line[0] != "#":
                break
            m = re.match(r"\s+(\w+)\s*=\s*new\s+", line)
            if m:
                signals.append(m.group(1))
    return signals

def generate(signals: list[str]) -> str:
    lines = [HEADER]
    for sig in signals:
...
if __name__ == "__main__":
    signals = extract_signals(CONNECTOR_SRC)
    SYSTEM_DST.write_text(generate(signals))
    print(f"Generated {SYSTEM_DST} with {len(signals)} signal connections")
```

#### Build integration

Add the validation build to `ato.yaml`:
...
builds:
  ...
  main:
    entry: elec/src/main.ato:Main
  system:
    entry: elec/src/system.ato:System   # validation only
```
...
Now `make hw-build` always regenerates the validation module before building. Adding a signal to `BoardConnectorInterface` automatically adds it to validation — no manual sync needed.

#### What the validation build catches

| Error | Single-board build | Validation build |
|---|---|---|
| Signal named differently on each board | No (separate builds) | Yes (shared interface, always caught by Strategy 1) |
| Signal wired to wrong internal net on one board | No | Yes (constraint mismatch across the `~` link) |
| Voltage/current constraint inconsistency across boards | No | Yes (atopile sees both sides of the constraint) |
| Signal present in connector but unwired on one board | No | Possibly (depends on atopile's floating-net detection) |
| Physical pin mismatch (header pin numbering) | No | No (still needs Strategy 2 netlist comparison) |

#### Limitations

- Depends on atopile being able to compile a module that instantiates two complete boards. If either board has components that conflict (e.g., two instances of a singleton part), the validation build may fail for reasons unrelated to the interface. This needs testing.
- The generated `system.ato` should be `.gitignore`'d (it's a build artifact) or checked in with a header warning that it's auto-generated.
- Only validates logical connections, not physical pin assignments. Strategy 2 (netlist comparison) is still needed for physical verification.
