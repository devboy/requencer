...
describe('clearGateStepsOnPage', () => {
  ...
  it('resets page 0 on a 16-step track with data on both halves', () => {
    let state = createSequencer()
    state = setGateOn(state, 0, 0, true)
    state = setGateOn(state, 0, 8, true) // second half — should be untouched
    const result = clearGateStepsOnPage(state, 0, 0)
    for (let i = 0; i < 16; i++) {
      expect(result.tracks[0].gate.steps[i]).toEqual({ on: false, tie: false, length: 0.5, ratchet: 1 })
    }
  ...
  it('handles reduced-length track correctly', () => {
    let state = createSequencer()
    state = setGateOn(state, 0, 0, true)
    state = setGateOn(state, 0, 5, true)
    state = setSubtrackLength(state, 0, 'gate', 8)
    const result = clearGateStepsOnPage(state, 0, 0)
    // Steps 0-7 should be cleared (active page)
    for (let i = 0; i < 8; i++) {
      expect(result.tracks[0].gate.steps[i]).toEqual({ on: false, tie: false, length: 0.5, ratchet: 1 })
    }
    // Length should be preserved
    expect(result.tracks[0].gate.length).toBe(8)
  })
...
describe('clearTrackToDefaults', () => {
  ...
  it('preserves lengths and dividers', () => {
    ...
    const result = clearTrackToDefaults(state, 0)
    expect(result.tracks[0].gate.length).toBe(8)
    expect(result.tracks[0].gate.steps.length).toBeGreaterThanOrEqual(8)
