...
export function renderHoldOverlay(ctx: CanvasRenderingContext2D, engine: SequencerState, ui: UIState, thinMode: boolean = false): void {
  ...
  if (thinMode) {
    ...
    if (held.kind === 'subtrack') {
      ...
      drawText(ctx, `LEN G:${track.gate.length} P:${track.pitch.length} V:${track.velocity.length}`, PAD, LCD_CONTENT_Y + 22, COLORS.textBright, 16)
