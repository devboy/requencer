/**
 * LCD Settings screen — sectioned parameter list for global clock & MIDI config.
 * Enc A scrolls params, Enc B adjusts values.
...
export function renderSettings(ctx: CanvasRenderingContext2D, engine: SequencerState, ui: UIState): void {
  ...
  for (let vi = 0; vi < maxVisible && scrollOffset + vi < visibleRows.length; vi++) {
    ...
    const isSelected = paramIdx === ui.settingsParam
    ...
    if (row.type === 'header') {
      ...
      if (isSelected) {
        drawText(ctx, '\u25B8', PAD, lineY + 5, COLORS.text, 16)
      }
    } else {
      // Param row
      ...
      if (isSelected) {
        fillRect(ctx, { x: PAD, y: y - 2, w: LCD_W - PAD * 2, h: ROW_H - 2 }, `${COLORS.text}22`)
      }

      // Cursor indicator
      const cursorColor = isSelected ? COLORS.text : 'transparent'
  ...
  if (visibleRows.length > maxVisible) {
    ...
    fillRect(ctx, { x: LCD_W - 3, y: thumbY, w: 2, h: thumbH }, `${COLORS.text}44`)
  }
}
