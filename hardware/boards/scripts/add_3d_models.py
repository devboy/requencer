#!/usr/bin/env python3
"""Add KiCad built-in 3D model references to manual footprint files and placed PCBs.

Two modes:
  1. Library footprints: Patches .kicad_mod files in hardware/boards/parts/
  2. Placed PCBs: Patches .kicad_pcb files to inject model refs into inline footprints
...
def patch_pcb_file(filepath: Path) -> int:
    """Inject or replace model references in footprint blocks in a .kicad_pcb file.

    Returns the number of footprints patched.
    """
    content = filepath.read_text()
    patched = 0
...
def patch_library_footprints(computed_offsets: dict[str, tuple]) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--pcb-only",
        action="store_true",
        help="Only patch placed PCB files (skip library footprints)",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if not args.pcb_only:
        patch_library_footprints(computed_offsets)
    patch_pcb_files()
